...
proc init_gui { IPINST } {
  ...
  set_property tooltip {Width of control bus address} ${AXI_ADDR_WIDTH}
  set BASEADDR [ipgui::add_param $IPINST -name "BASEADDR" -parent ${Page_0}]
  set_property tooltip {Please set this equal to the address used in the Address Editor} ${BASEADDR}
  ...
  set_property tooltip {If packet filter bandiwdth is your bottleneck, increase this number} ${N}
  set PACKET_BYTE_ADDR_WIDTH [ipgui::add_param $IPINST -name "PACKET_BYTE_ADDR_WIDTH" -parent ${Page_0}]
  set_property tooltip {The capacity of the packet buffers is 2 to the power of this number (in bytes)} ${PACKET_BYTE_ADDR_WIDTH}
  set PESSIMISTIC [ipgui::add_param $IPINST -name "PESSIMISTIC" -parent ${Page_0}]
  set_property tooltip {Enables a few internal registers. Greatly eases timing, slightly reduces performance} ${PESSIMISTIC}
  set SNOOP_FWD_ADDR_WIDTH [ipgui::add_param $IPINST -name "SNOOP_FWD_ADDR_WIDTH" -parent ${Page_0}]
  set_property tooltip {The width of the snooper and forwarder data is 2 to the power of (byte address width minus this number) in bytes} ${SNOOP_FWD_ADDR_WIDTH}
